                ...
                if (authCookie != null)
                {
                    ...
                    if (jsonToken != null)
                    {
                        ...
                        var principal = new CustomPrincipal(claims.First(c => c.Type == JwtRegisteredClaimNames.UniqueName).Value)
                        {
                            ...
                            Permissions = JsonConvert.DeserializeObject<IEnumerable<PermissionRes>>(claims.FirstOrDefault(c => c.Type == "/Permissions")?.Value) // Add your logic to parse permissions, if they are in the claims
